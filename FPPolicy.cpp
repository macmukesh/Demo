...
class FPPolicy : private ReaderPolicy, private ParserPolicy,private StoragePolicy
{
    ...
    using ParserPolicy::ValidateInputRawData ;
		...
		using StoragePolicy::GetTankLastSeqNo;

		// tank seqno array
		static const int MAX_NUM_TANK=10;
		unsigned long lTankSeqNo[10];
...
public:
    ...
    bool run(const char * filenm)
    {
				...
				{
					lTankSeqNo[ix]=0;										
					lTankSeqNo[ix]=GetTankLastSeqNo(ix);	
				}
		...
		void ProcessBuffer(vector<ArrBufferData> & vecBuf,int iBuffSz)
		{
				...
				for(int iX=0;iX<iBuffSz;++iX)
				{
						...
						bool bret=ParseDataIntoStruct(vecBuf[iX].acRawdata,vecBuf[iX].iRowSz,vecBuf[iX].enParserCd,vecBuf[iX].ObjTT,strErrDesc);	
						
						if(bret)
						StoreBuffer(vecBuf[iX]);	
						
						// if error then write into reject file
						if(!bret)
						{
								// write into reject file ..strErrDesc
						}
				}
		}
		...
		{	
			...
			StoreRecordIntoMem(Buf.ObjTT,Buf.lTankSeq);
		}
		...
		void AddTxnRow(const char * data,const unsigned int irowsz,vector<ArrBufferData> & vecData)
		{
			...
			// set parsercd to default
			ObjRowData.enParserCd=INVALID_PARSERCODE;
			// identify parsercd and validation ...objpc will point to invalid_Pc in case of error
			IdentifyParserCd(ObjRowData.acRawdata,ObjRowData.iRowSz,ObjRowData.enParserCd);

			// get tank row no and paste into output..only financial txn
			ObjRowData.lTankSeq=lTankSeqNo[ObjRowData.enParserCd];
			++lTankSeqNo[ObjRowData.enParserCd];
